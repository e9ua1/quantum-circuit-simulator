...
public class RedundantGateRemover implements CircuitOptimizer {
    ...
    public QuantumCircuit optimize(QuantumCircuit circuit) {
        ...
        List<CircuitStep> optimizedSteps = removeRedundantGates(steps);
        return buildOptimizedCircuit(circuit, optimizedSteps);
    }

    private List<CircuitStep> removeRedundantGates(List<CircuitStep> steps) {
    ...
    private boolean shouldSkipPair(List<CircuitStep> steps, int index) {
        ...
        return areConsecutiveSameGates(current, next);
    }

    private boolean areConsecutiveSameGates(CircuitStep step1, CircuitStep step2) {
